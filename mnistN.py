...
model = NVAE(28*28,128, args.z_dim)
...
for i in tqdm(range(1000)):
    ...
    for X,y in train_loader:
        ...
        loss = nn.MSELoss(reduction='none')(x_, X).sum(-1).mean()
        loss += torch.distributions.kl.kl_divergence(q_z, p_z).sum(-1).mean()
    ...
    for X, y_mb in test_loader:
        ...
        X = X.reshape(-1,784)
        (z_mean, z_var), (q_z, p_z), z, x_ = model(X)
        val_loss += nn.MSELoss(reduction='none')(x_, X).sum(-1).mean().item()
        val_loss+=torch.distributions.kl.kl_divergence(q_z, p_z).sum(-1).mean().item()
    ES(val_loss)
    if ES.early_stop:
        print("Early stopping")
        break

torch.save(model.state_dict(),"NVAE"+str(args.z_dim))